...
namespace InformationRetrievalManager.Core
{
    ...
    public class BaseTaskManager : ITaskManager
    {
        ...
        {
            FrameworkDI.Logger.LogErrorSource($"An unexpected error occured running a TaskManager: ({ex.GetType().ToString()}) {ex.Message}", origin: origin, filePath: filePath, lineNumber: lineNumber);
        }
